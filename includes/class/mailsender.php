...
class MailSender
{
    ...
    {   
        ...
        require ($signin_url.'login/scripts/PHPMailer/PHPMailerAutoload.php');
        require ($signin_url.'login/config.php');
        ...
        *****/
        ...
        if ($type == 'Verify') {
            ...
            $url = crear_url($id);
            $mail->Subject = $user . ' Cambiar Contraseña!';

            //Set the body of the message
            $mail->Body = $pass_reset . '<br><a href="'.$url.'">'.$url.'</a>';
            $mail->AltBody  =  $pass_reset . $url;
            $url_jj = $url;
